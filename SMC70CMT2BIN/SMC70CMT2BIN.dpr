...
begin
  ...
  begin
    ...
    // シンボル長が0/1の中心からどれだけ外れているかを
    // 候補評価に使用する。0/1の中心は約29.5/39.5sample。
...
begin
  ...
  // 現行のSMC-70実機録音では、レコード直前のリーダーが
  // 録音系によって著しく崩れる場合がある。
  // そのため、リーダー10組の存在を必須条件にせず、
  // 「128バイト+チェックサムが成立するデータ列」を探索する。
  //
  // ただし長いZERO領域も同じ条件を満たし得るので、
  // 非ZEROバイト数を優先し、その次にシンボルタイミングを評価する。
...
begin
  ...
  // 従来方式で見つからない場合は、実機録音向けの
  // チェックサム付きデータ列探索へフォールバック。
...
begin
  ...
  // ------------------------------------------------------------
  //
  // アドレス範囲指定やOutput Offsetは使用しない。
  // 標準ラベルのロード長で復号したCMTデータを
  // 先頭から最後まで出力する。
...
begin
  ...
  Writeln(
    'SMC70CmtDecode.exe input.wav output.bin'
  );
  ...
  Writeln(
    '  SMC70CmtDecode.exe test4.wav test4.bin'
  );

  Writeln;
  Writeln(
    '入力WAVのCMTデータを標準ラベルのロード長に従って'
  );

  Writeln(
    '先頭から最後まで復号し、output.binへ出力します。'
  );
